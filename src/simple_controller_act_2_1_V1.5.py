...
# HOG detection for pedestrians - detectoin window
#     
def detect_pedestrians_svm(frame, pedestrian_model, roi_y_start=0.35, step=54):
    height, width, _ = frame.shape
    detections = []
    ...
    for win_w, win_h in PEDESTRIAN_SCALES:
        for y in range(y_start, y_end - win_h, step):
            for x in range(x_start, x_end - win_w, step):

                crop = frame[y:y + win_h, x:x + win_w]

                features = extract_pedestrian_hog_features(crop)
...
def main():
    ...
    while robot.step() != -1:
        ...
        # ==========================================
        # RUN HEAVY SVM DETECTION ONLY EVERY 10 FRAMES
        # ==========================================
