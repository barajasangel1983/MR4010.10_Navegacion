...
#Sign finder with CNN based in webots all frame
# ============================================================
...
#   1. Search only a large ROI where signs normally appear.
#   2. Use OpenCV color/contour heuristics to find a sign candidate.
#   3. Crop tightly around that candidate.
#   4. Send only the tight crop to the CNN classifier.
#
# This fixes the previous issue where the CNN saw a large ROI/full frame and
# learned mostly background instead of the sign itself.
# ==============================
...
def detect_sign_candidates(img_bgr, max_candidates=4):
    """
    Find up to max_candidates traffic-sign candidate boxes inside img_bgr.

    Returns all valid boxes sorted by area descending so the caller can
    classify each one and keep the best result, rather than committing to
    a single largest box that might be background.

    Returns:
        boxes: list of (x, y, w, h) in local ROI coordinates (may be empty)
        mask:  binary mask used for debugging
    """
    hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)

    # Red hue wraps around 0/180 in HSV — two ranges needed.
    red_mask_1 = cv2.inRange(hsv, np.array([0,   30, 40]), np.array([15,  255, 255]))
    red_mask_2 = cv2.inRange(hsv, np.array([160, 30, 40]), np.array([180, 255, 255]))
    red_mask   = cv2.bitwise_or(red_mask_1, red_mask_2)

    # Tight white mask — only truly white pixels (very low saturation, very high value).
    # The previous range [S:0-110, V:100-255] matched the whole Webots scene
    # (sky, buildings, roads all have low saturation in Webots rendering).
    # S < 40 and V > 180 selects only sign faces and white road markings.
    ...
    for contour in contours:
        ...
        # Lowered from 500 — distant signs appear small in Webots.
        if area < 150:
            continue
        ...
        # Widened to accept taller or wider sign boards.
        if aspect_ratio < 0.15 or aspect_ratio > 4.0:
            continue

        # Border filter removed — signs near the ROI edge were being dropped.

        valid_boxes.append((x, y, w, h, area))

    valid_boxes.sort(key=lambda b: b[4], reverse=True)
    boxes = [(x, y, w, h) for x, y, w, h, _ in valid_boxes[:max_candidates]]
...
def classify_sign_frame(frame_bgr, sign_model, sign_class_names):
    """
    Hybrid pipeline: ROI → multiple candidates → classify each → keep best non-no_sign.

    Returns:
        label:          predicted class name
        confidence:     model probability for that class
        sign_box:       tight crop box in full-frame coordinates (None if no sign found)
        search_roi_box: green search ROI in full-frame coordinates
    """
    ...
    # ── DEBUG WINDOW ──────────────────────────────────────────────────────────
    # Shows the ROI crop and the binary mask side by side.
    # Left = what the heuristic sees; Right = what the color filter detected.
    # Use this to tune HSV thresholds and area filters. Remove when done.
    debug_roi   = cv2.resize(roi, (320, 200))
    debug_mask  = cv2.resize(candidate_mask, (320, 200))
    debug_mask3 = cv2.cvtColor(debug_mask, cv2.COLOR_GRAY2BGR)
    # Draw found candidate boxes on the ROI panel.
    ...
    cv2.imshow("Sign Heuristic Debug", np.hstack([debug_roi, debug_mask3]))
    # ─────────────────────────────────────────────────────────────────────────
    ...
    return best_label, best_confidence, best_sign_box, search_roi_box


def draw_sign_detection(debug_frame, label, confidence, sign_box, search_roi_box=None):
    """
    Draw the sign detection result.

    Green box  = large search ROI.
    Blue box   = tight candidate crop sent to CNN.
    Text       = final CNN classification.
    """
    # Draw large search ROI in green to show where the heuristic looks.
    ...
    # If there is no candidate, only show no_sign status.
    if sign_box is None:
        cv2.putText(
            debug_frame,
    ...
    # Blue candidate box for sign detections. Green if model says no_sign.
    color = (0, 255, 0) if label == "no_sign" else (255, 0, 0)
...
def main():
    ...
    # Last known sign classification result. These values are updated
    # periodically, not every frame, to avoid unnecessary CNN inference load.
    sign_label = "no_sign"
    sign_confidence = 0.0
    sign_box = None
    sign_search_roi_box = None
    ...
    while robot.step() != -1:
        ...
        if (
            ...
            sign_label, sign_confidence, sign_box, sign_search_roi_box = classify_sign_frame(
                frame,
                sign_model,
                sign_class_names
            )
