# ============================================================
# V3.0 — Webots Self-Driving Controller
# ============================================================
#
# WHAT IS NEW IN V3.0 vs V2.0
# ──────────────────────────────
#   1. CIL model reverted to Notebook 28 architecture (cil_model.pt):
#        - NB28 uses torch.jit.script export — forward(x, command: int).
#        - nav_command passed as Python int (not LongTensor).
#        - NB28 output is a raw unbounded angle; CIL_ANGLE_CLAMP still applies.
#        - Preprocessing pipeline is identical — no changes to _cil_preprocess.
#
# WHAT IS NEW IN V2.0 vs V1.16
# ──────────────────────────────
#   1. CIL model updated to Notebook 30 architecture:
#        - NB30 uses torch.jit.trace export (vs NB28's torch.jit.script).
#        - NB30 forward() expects nav_command as torch.Tensor(dtype=long),
#          not a Python int. _cil_predict_smooth updated accordingly.
#        - NB30 output is pre-scaled by Tanh × 0.7 — CIL_ANGLE_CLAMP (0.70)
#          is still correct and harmless.
#        - Preprocessing pipeline is identical — no changes to _cil_preprocess.
#
# WHAT IS NEW IN V1.16 vs V1.15
# ──────────────────────────────
#   1. PID_ENABLED runtime toggle (key L):
#        - PID_ENABLED = True  by default (same behavior as V1.15).
#        - Press L to disable PID entirely; the controller falls through to
#          bc_evasion_model Phase 0 even when a lane reference is visible.
#        - ADAS monitor "Control" row reflects the disabled state.
#
#   2. BC_ENABLED runtime toggle (key B):
#        - BC_ENABLED = True  by default.
#        - Press B to disable the bc_evasion_model; Phase 1 EVADING and Phase 0
#          line-lost both hold the last angle instead of running inference.
#        - ADAS monitor "BC evasion" row shows "DISABLED" in grey.
#
#   3. CIL_ENABLED runtime toggle (key I):
#        - CIL_ENABLED = True  by default.
#        - Press I to disable CIL; nav_commands (L1/R1/O) are ignored and the
#          car stays on PID/BC through intersections.
#        - Disabling resets the CIL state machine and nav_command immediately.
#        - ADAS monitor "CIL model" row shows "DISABLED" in grey.
#
...
#     Triangle    — toggle CIL LANE_FOLLOW mode (CIL cmd 0 replaces PID; key T on keyboard)
...
BTN_TRIANGLE = 2   # Triangle — toggle CIL LANE_FOLLOW mode
...
# ==============================
# PID / BC / CIL RUNTIME TOGGLES
# ==============================

# Each flag can be flipped at runtime via keyboard (L / B / I).

# PID — key L
# When False the controller skips PID even if a lane reference is visible and
# falls through to bc_evasion_model Phase 0 (or hold-angle if model not loaded).
PID_ENABLED = False

# BC — key B
# Active only during Phase 1 EVADING — bc_evasion_model steers around the obstacle.
# Phase 0 line-lost BC is unreachable while CIL_LANE_FOLLOW_ENABLED = True.
BC_ENABLED = True

# CIL — key I
# When False nav_commands are ignored; the car stays on PID/BC through
# intersections. Disabling also resets the CIL state machine immediately.
CIL_ENABLED = True

# CIL LANE_FOLLOW — PS3 Triangle button / key T
# Always-on: cil_model_V3.pt cmd=0 is the default steering controller.
# Intersection commands (L1/R1/Circle) arm and override when _cil_executing.
# Triangle/T can still toggle it off for debugging.
CIL_LANE_FOLLOW_ENABLED = True
...
# Path to the TorchScript CIL model — NB28 architecture (torch.jit.script).
...
def _cil_predict_smooth(model, frame_bgr, nav_command, prev_smoothed):
    ...
    # NB30 model expects nav_command as a LongTensor([cmd]) on the same device,
    # not a Python int (NB28 used torch.jit.script with command: int annotation).
    # NB28 model uses torch.jit.script with forward(x: Tensor, command: int).
    with _torch.no_grad():
        raw = float(model(tensor, int(nav_command)).item())
...
def draw_adas_monitor(speed, angle, brake, gps_vals, gyro_vals,
                      ...
                      bc_enabled=True, cil_enabled=True,
                      cil_lane_follow_enabled=False, cil_lane_follow_active=False):
    ...
    elif cil_active and cil_lane_follow_active:
        ctrl_str, ctrl_color = "CIL  LANE_FOLLOW", (0, 220, 255)
    elif cil_active:
    ...
    if not PID_ENABLED and _has_lane_ref and bc_active:
        ctrl_str, ctrl_color = "BC  (PID disabled)", (255, 165, 0)
    elif not _has_lane_ref and bc_active:
    ...
    if not bc_enabled:
        bc_row_str, bc_row_color = "DISABLED",                    (80, 80, 80)
    elif bc_active:
    ...
    # CIL row
    if not cil_enabled:
        cil_row_str, cil_row_color = "DISABLED", (80, 80, 80)
    elif cil_active and cil_lane_follow_active:
        cil_row_str, cil_row_color = "ACTIVE  (LANE_FOLLOW)", (0, 220, 255)
    elif cil_active:
        cil_row_str   = f"ACTIVE  ({_CIL_CMD_NAMES.get(nav_command, '?')})"
        cil_row_color = (255, 200, 0)
    elif cil_lane_follow_enabled and cil_model_loaded:
        cil_row_str, cil_row_color = "LANE_FOLLOW  (standby)", (0, 180, 120)
    ...
    _nav_colors = {
        0: (0, 220, 255) if cil_lane_follow_active else (0, 180, 120) if cil_lane_follow_enabled else (100, 100, 100),
        1: (0, 255, 0),
...
def main():
    global DEBUG_PANEL, ADAS_MONITOR_ENABLED, PID_ENABLED, BC_ENABLED, CIL_ENABLED, CIL_LANE_FOLLOW_ENABLED
    speed = 0
    angle = 0.0
    brake = 0.0
    ...
    _cil_lane_follow_active = False   # True specifically when LANE_FOLLOW branch fired
    ...
    print("=" * 60)
    print("  Webots Self-Driving Controller — V3.0")
    print("=" * 60)
    print("  Keyboard controls:")
    print("    S          — toggle autonomous mode")
    print("    L          — toggle PID (ON/OFF)")
    print("    B          — toggle BC evasion model (ON/OFF)")
    print("    I          — toggle CIL intersection model (ON/OFF)")
    ...
    while robot.step() != -1:
        ...
        if key != -1:
            if key not in last_press or current_time - last_press[key] > DEBOUNCE_TIME:
                ...
                if key == keyboard.UP:
                    ...
                    if not PID_ENABLED:
                        pid.reset()
                    print(f"PID: {'ON' if PID_ENABLED else 'OFF'}")

                elif key in (ord("B"), ord("b")):
                    BC_ENABLED = not BC_ENABLED
                    print(f"BC: {'ON' if BC_ENABLED else 'OFF'}")

                elif key in (ord("I"), ord("i")):
                    CIL_ENABLED = not CIL_ENABLED
                    if not CIL_ENABLED:
                        _cil_executing = False
                        _cil_exec_start = None
                        _cil_exit_count = 0
                        dataset_mode.reset_nav_command()
                    print(f"CIL: {'ON' if CIL_ENABLED else 'OFF'}")

                elif key in (ord("T"), ord("t")):
                    CIL_LANE_FOLLOW_ENABLED = not CIL_LANE_FOLLOW_ENABLED
                    if not CIL_LANE_FOLLOW_ENABLED:
                        _cil_smoothed_angle = 0.0
                    print(f"CIL LANE_FOLLOW: {'ON' if CIL_LANE_FOLLOW_ENABLED else 'OFF'}")
        ...
        COLOR_VEHICLE       = (0, 0, 255)     # Red    — vehicles (bus, car, truck…)
        COLOR_TRAFFIC_LIGHT = (0, 255, 255)   # Yellow — traffic lights
        COLOR_SIGN          = (255, 255, 0)   # Cyan   — road signs
        COLOR_OBSTACLE      = (255, 0, 0)     # Blue   — cones, barrels, barriers
        COLOR_PEDESTRIAN    = (0, 165, 255)   # Orange — pedestrians
        ...
        CATEGORIES = {
            ...
            "pedestrian":    ["pedestrian"],   # matches pedestrian / pedestrian(1) / pedestrian(2)
        }
        ...
        if num_obj > 0 and ENABLE_OBJECT_DETECTION:
            ...
            for obj in objects:
                ...
                elif any(c in model_lower for c in CATEGORIES["pedestrian"]):
                    obj_type = "pedestrian"
        ...
        if lidar_obstacle and lidar_obstacle_distance is not None:
            if lidar_obstacle_distance < LIDAR_STOP_DISTANCE:
                ...
                elif any(o["type"] == "pedestrian" for o in nearby):
        ...
        if autonomous_mode:
            ...
            if evasion_phase == 1:
                ...
                if not _left_ds_triggered:
                    ...
                    if bc_model is not None and BC_ENABLED:
                        try:
                        ...
                        # BC model not loaded or disabled — hold the last angle to maintain trajectory
                        angle = previous_angle
                    cv2.putText(debug_frame,
            ...
            # only runs for phase 0 (normal / sharp turn situations).
                ...
                if lidar_obstacle_distance < LIDAR_STOP_DISTANCE:
                    ...
                    elif CIL_ENABLED and CIL_LANE_FOLLOW_ENABLED and cil_model is not None:
                        # CIL LANE_FOLLOW steers at half speed while obstacle is in slow zone.
                        try:
                            _cil_smoothed_angle = _cil_predict_smooth(
                                cil_model, frame, dataset_mode.CMD_LANE_FOLLOW, _cil_smoothed_angle)
                            angle          = _cil_smoothed_angle
                            previous_angle = angle
                            cv2.putText(debug_frame,
                                        f"AUTO | SLOW  CIL LANE_FOLLOW  angle:{angle:.3f}  dist:{lidar_obstacle_distance:.1f}m",
                                        (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 220, 255), 2)
                        except Exception as _e:
                            print(f"[CIL] Inference error: {_e} — holding angle={previous_angle:.3f}")
                            angle = previous_angle
                    else:
                        # No CIL model — hold last angle.
                        angle = previous_angle
                        cv2.putText(debug_frame,
                                    f"AUTO | SLOW  HOLD  angle:{angle:.3f}  (no model)",
            ...
            # Reached only when evasion_phase=0 and no LiDAR emergency.
                ...
                if CIL_ENABLED and _nav_cmd != dataset_mode.CMD_LANE_FOLLOW and _cil_executing and cil_model is not None:
                    cil_active              = True
                    _cil_lane_follow_active = False
                    bc_active               = False
                    brake                   = 0.0
                    speed                   = CIL_SPEED
                    try:
                ...
                # ── PRIORITY 2b: CIL LANE_FOLLOW — Triangle replaces PID ─────
                # Fires only when no intersection maneuver is executing.
                # Model runs with cmd=0; intersection commands (L1/R1/O) still
                # arm normally and take over once _cil_executing flips True.
                elif CIL_ENABLED and CIL_LANE_FOLLOW_ENABLED and cil_model is not None:
                    cil_active              = True
                    _cil_lane_follow_active = True
                    bc_active               = False
                    brake                   = 0.0
                    speed                   = CIL_SPEED
                    try:
                        ...
                        previous_angle = angle
                        cv2.putText(debug_frame,
                                    f"AUTO | CIL LANE_FOLLOW  angle:{angle:.3f}",
                                    (30, 40), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.8, (0, 220, 255), 2)
                    except Exception as _e:
                        print(f"[CIL] Inference error: {_e} — holding angle={previous_angle:.3f}")
                    ...
                    if PID_ENABLED and (line_detected or (lane_side == dataset_mode.LANE_RIGHT and edge_detected)):
                        bc_active = False
        ...
        if ADAS_MONITOR_ENABLED:
            draw_adas_monitor(speed, angle, brake, gps_vals, gyro_vals,
                              ...
                              bc_enabled=BC_ENABLED, cil_enabled=CIL_ENABLED,
                              cil_lane_follow_enabled=CIL_LANE_FOLLOW_ENABLED,
                              cil_lane_follow_active=_cil_lane_follow_active)
