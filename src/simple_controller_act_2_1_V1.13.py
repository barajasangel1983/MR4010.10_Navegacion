# ============================================================
# V1.13 — Webots Self-Driving Controller
# ============================================================
#
...
#      Used both for driving control and as a dataset label.
#   2. Right-lane PID mode: when lane_side = LANE_RIGHT, PID uses edge_error
#      from detect_gravel_edge instead of yellow-line error from detect_yellow_line.
#   3. lane_side toggle — key N switches between LEFT_LANE and RIGHT_LANE.
#      Affects which reference (yellow line vs gravel edge) drives the PID.
#   4. Navigation commands via PS4 (CIL intent labeling + future CIL inference):
#        L1 → CMD_LEFT    (turn left at next intersection)
#        R1 → CMD_RIGHT   (turn right at next intersection)
#        O  → CMD_STRAIGHT (cross next intersection straight)
#      Command latches until reset after intersection (edge/line reacquired).
#   5. Dataset capture upgraded to dataset_mode_v5:
#        - New columns: lane_side, edge_detected, nav_command
#        - 4 Hz capture when nav_command is active, 2 Hz otherwise
#        - New directory: behavioral_dataset_CIL_06242026
#   6. ADAS Monitor updated with lane_side, edge_detected, and nav_command rows.
#   7. All V1.12 features retained: BC inference, PID toggle, BC/PID arbitration,
#      evasion phases, stuck detection, dataset_mode_v5 capture.
#
# BC / PID ARBITRATION — four operating modes
# ─────────────────────────────────────────────
#   B=ON  L=ON   HYBRID  — PID handles class 0 (normal lane following with
#                           yellow-line feedback); BC handles classes 1/2/3/4
#                           (evasion, returning, sharp turns). Best of both.
#
#   B=ON  L=OFF  FULL BC — BC controls all five classes including normal
#                           driving. Use this to validate the BC model on the
#                           full circuit without any PID assistance.
#
#   B=OFF L=ON   PURE PID — V1.11 behaviour. BC model is loaded but never
#                           called. Safe baseline for comparison.
#
#   B=OFF L=OFF  SAFE HOLD — Both controllers disabled. Angle freezes at 0,
#                           speed drops to 0, gentle brake applied. Emergency
#                           fallback only — should not occur in normal use.
#
# BC MODEL PIPELINE (per frame, when BC is active)
# ─────────────────────────────────────────────────
#   1. Crop frame: top 38 % (sky) and bottom 12 % (hood) removed.
#   2. Resize to 66 × 200 px — NVIDIA DAVE-2 input size.
#   3. Normalize to [0.0, 1.0] float32.
#   4. Run TorchScript inference (GPU if available, otherwise CPU).
#   5. Clamp raw output to ± BC_ANGLE_CLAMP (0.70 rad) to guard against
#      extreme predictions on out-of-distribution frames.
#   6. Apply Exponential Moving Average: smoothed = α × raw + (1−α) × prev.
#      BC_SMOOTH_ALPHA = 0.40 gives ~80 ms lag at 32 ms timestep — removes
#      single-frame jitter without slowing the car's response to real curves.
#
# BC SPEED BY CLASS
# ─────────────────
#   All speeds in km/h. Edit BC_SPEED_BY_CLASS in the config section below.
#   Classes 3 and 4 (sharp turns) default to 15 km/h to allow safe cornering
#   at intersections. Classes 0/1/2 default to 30 km/h.
#
# BC STUCK DETECTION
# ──────────────────
#   Every frame, while BC is in control and autonomous mode is active, the
#   controller reads driver.getCurrentSpeed() (actual vehicle speed, not the
#   commanded setpoint). If actual speed < BC_STUCK_SPEED_THRESHOLD (5 km/h)
#   for more than BC_STUCK_TIME_THRESHOLD (5 s) while BC is commanding > 5 km/h,
#   a WARNING is printed to the console. The timer resets after each warning
#   so the alert repeats every 5 s until the car starts moving again.
#   Action: press S to exit autonomous mode and manually unstick the vehicle.
#
# LANE FOLLOWING (PID — unchanged from V1.11)
# ─────────────────────────────────────────────
...
#     then revert automatically to MANUAL mode. (Only active when BC is OFF.)
#
# LIDAR OBSTACLE DETECTION  (Sick LMS 291)
# ──────────────────────────────────────────
#   - Front-center 40° inspection window (±20 samples from center).
#   - Max detection range: 25 m; emergency-stop threshold: 10 m.
#   - Between 10–25 m: slow to 50 % speed, lane following stays active.
#   - Below 10 m: full brake, zero steering, classify obstacle type.
#
...
#   - Only camera objects within LIDAR_STOP_DISTANCE (10 m) trigger evasion.
#
...
# ────────────────────────────────────────────────────
#   Classes are auto-detected from sensor state — no manual annotation needed.
#   Priority order: evasion (1/2) > sharp turn (3/4) > normal (0).
#   Sharp turn entry: angle > 0.25 rad + no yellow line + no evasion active.
#   Sharp turn dwell: class 3/4 held for 4 s after entry regardless of angle.
#
#   Class | Name           | Trigger
#   ------+----------------+----------------------------------------
...
#   - MACHINE STATE section now includes BC and PID status rows.
...
#   - Displays: "wZ align" row only during RETURNING phase.
#
# DATASET CAPTURE  (dataset_mode_v4 — Behavioral Cloning)
# ─────────────────────────────────────────────────────────
#   - Toggle with keyboard D or PS4 Triangle.
#   - Captures at 2 Hz after all control decisions so angle/speed/brake reflect
#     the actual command applied this frame (BC or PID — transparent to dataset).
#   - Works in manual AND autonomous mode.
#   - Multi-contributor: each team member sets CONTRIBUTOR_ID in dataset_mode_v4.py
#     or via $env:DATASET_CONTRIBUTOR_ID before running Webots.
#   - CSV columns: contributor_id, session_id, timestamp, image_filename,
#     steering_angle, speed_kmh, brake, autonomous_mode, line_detected,
#     gps_x, gps_y, lidar_obstacle_detected, lidar_obstacle_distance,
#     behavior_class, obstacle_type.
#   - Images → data/behavioral_dataset_06232026/images/{ID}_frame_XXXXXX.jpg
#   - Metadata → data/behavioral_dataset_06232026/measurements.csv (append mode)
#   - Frame counter resumes from existing files — toggling D never overwrites.
#
# INPUT / CONTROL
# ────────────────
#   Keyboard:
#     Arrow keys  — manual speed / steering
...
#     C           — save single camera image to file
#     D           — toggle dataset capture mode
#     P           — toggle debug panel (Self Driving Debug + Yellow Mask windows)
#     Q           — quit
#   PS4 controller:
#     Left stick  — steering (manual only)
#     R2          — throttle
#     L2          — brake / reverse
#     X button    — toggle autonomous mode
#     Square      — save image
#     Triangle    — toggle dataset capture mode
#     L1          — nav_command = LEFT  (turn left at next intersection)
#     R1          — nav_command = RIGHT (turn right at next intersection)
#     O (Circle)  — nav_command = STRAIGHT (cross next intersection straight)
#   Debounce: 0.1 s keyboard, 0.5 s PS4 buttons.
#
# DEBUG DISPLAYS
# ───────────────
#   - "Self Driving Debug": annotated camera view with line, lane center, HUD,
#     obstacle status, behavior class, BC/PID mode, and dataset indicator.
#   - "Yellow Mask Debug" / "Gravel Edge Debug": swaps based on lane_side (N key).
#     LEFT lane  → yellow binary mask  (tune yellow line detection).
#     RIGHT lane → gravel edge binary mask (tune EDGE_BRIGHT_THRESH).
#   - "PID Response Chart": live error (px), steering (rad), and speed (km/h) traces.
#   - "ADAS Monitor": full sensor dashboard.
#   - Speed overlay color: cyan = autonomous, green = manual.
#   - DEBUG_PANEL (key P) gates Self Driving Debug and Yellow Mask windows.
...
_BC_DEVICE = None   # set once at load time, used by all inference calls
...
def draw_adas_monitor(speed, angle, brake, gps_vals, gyro_vals,
    ...
    # Panel height: V1.11=833, V1.12=879 (+46), V1.13=948 (+69 for 3 new rows)
    W, H = 400, 948
    ...
    data_row(187, "Lane side", ls_str, ls_color)
    ...
    data_row(210, "Gravel edge", ed_str, ed_color)
    ...
    data_row(233, "Nav cmd", _nav_labels.get(nav_command, "?"),
             _nav_colors.get(nav_command, (210, 210, 210)))

    # Yellow line — shifted +69 px from V1.12
    ...
    data_row(256, "Yellow line", line_str, line_color)

    # wZ alignment row — only visible during RETURNING; shifted +69 px
    if evasion_phase == 2 and saved_gyro_wz is not None and current_gyro_wz is not None:
        ...
        data_row(279, "wZ align", wz_str, wz_color)

    cv2.line(panel, (8, 293), (W - 8, 293), (50, 50, 50), 1)

    # ── VEHICLE STATE — shifted +69 px ────────────────────────────────────
    section_header(306, "VEHICLE STATE")
    spd_color = (0, 255, 0) if speed < 60 else (0, 165, 255)
    brk_color = (0, 0, 255) if brake > 0.1 else (210, 210, 210)
    data_row(328, "Speed",  f"{speed:.1f}  km/h", spd_color)
    data_row(351, "Angle",  f"{angle:.4f}  rad")
    data_row(374, "Brake",  f"{brake:.2f}", brk_color)

    # ── GPS — shifted +69 px ──────────────────────────────────────────────
    section_header(402, "GPS")
    if gps_vals is not None:
        data_row(424, "X", f"{gps_vals[0]:.3f}  m")
        data_row(447, "Y", f"{gps_vals[1]:.3f}  m")
        data_row(470, "Z", f"{gps_vals[2]:.3f}  m")
    else:
        data_row(434, "Status", "NOT AVAILABLE", (0, 80, 255))

    # ── GYRO — shifted +69 px ─────────────────────────────────────────────
    section_header(498, "GYRO")
    if gyro_vals is not None:
        data_row(520, "wX", f"{gyro_vals[0]:.4f}  rad/s")
        data_row(543, "wY", f"{gyro_vals[1]:.4f}  rad/s")
        data_row(566, "wZ", f"{gyro_vals[2]:.4f}  rad/s")
    else:
        data_row(530, "Status", "NOT AVAILABLE", (0, 80, 255))

    # ── DISTANCE SENSORS — shifted +69 px ─────────────────────────────────
    section_header(594, "DISTANCE SENSORS")
    ...
    data_row(616, "DS_ML  (mid-left)",    ml_str, ds_color(ds_ml_val))
    data_row(639, "DS_MR  (mid-right)",   mr_str, ds_color(ds_mr_val))
    data_row(662, "DS_FL  (front-left)",  fl_str, ds_color(ds_fl_val))
    data_row(685, "DS_FR  (front-right)", fr_str, ds_color(ds_fr_val))
    data_row(708, "DS_RL  (rear-left)",   rl_str, ds_color(ds_rl_val))
    data_row(731, "DS_RR  (rear-right)",  rr_str, ds_color(ds_rr_val))

    # ── LIDAR — shifted +69 px ────────────────────────────────────────────
    section_header(759, "LIDAR  (Sick LMS 291)")

    if lidar_obstacle and lidar_distance is not None:
        dist_color = (0, 0, 255) if lidar_distance < LIDAR_STOP_DISTANCE else (0, 165, 255)
        data_row(781, "Status",   "OBSTACLE DETECTED", dist_color)
        data_row(804, "Distance", f"{lidar_distance:.2f}  m", dist_color)
        ang_str = f"{lidar_angle:.1f}  deg" if lidar_angle is not None else "--"
        data_row(827, "Angle",    ang_str, (210, 210, 210))
        type_color = (0, 100, 255) if obstacle_type == "VEHICLE" else (210, 210, 210)
        data_row(850, "Type",     obstacle_type if obstacle_type else "--", type_color)
        vh_str = "YES" if vehicle_nearby else "NO"
        vh_color = (0, 0, 255) if vehicle_nearby else (0, 200, 0)
        data_row(873, "Vehicle",  vh_str, vh_color)
    else:
        data_row(781, "Status",   "CLEAR", (0, 255, 0))
        data_row(804, "Distance", "--")
        data_row(827, "Angle",    "--")
        data_row(850, "Type",     "--")
        data_row(873, "Vehicle",  "NO", (0, 200, 0))
...
def main():
    ...
    print("=" * 60)
    print("  Webots Self-Driving Controller — V1.13")
    print("=" * 60)
    ...
    while robot.step() != -1:
        ...
        if autonomous_mode:
            ...
            # ── Normal driving — four-mode BC/PID arbitration ─────────────────
            # Reached only when evasion_phase=0 and no LIDAR emergency.
                ...
                # Best of both worlds: PID has direct yellow-line feedback for
                # straight-road accuracy; BC handles the maneuver classes that
                # PID cannot (no line visible, sharp angles, evasion/return).
                if bc_enabled and bc_model is not None and pid_enabled \
                ...
                # Runs when:
                #   - HYBRID: line lost, or behavior_class is 3/4 (sharp turn)
                #   - FULL BC (pid_enabled=False): BC handles everything including class 0
                elif bc_enabled and bc_model is not None:
                    bc_active    = True
                ...
                # Identical to V1.11 behaviour. BC model is loaded but never called.
                elif pid_enabled:
                    bc_active = False
                ...
                # Should not occur in normal operation. Emergency fallback only.
                # Press S to exit autonomous mode and manually reposition the car.
                else:
                    bc_active = False
                    angle = 0.0
                    speed = 0.0
