...
##NOTES 06/06/2026
# CNN classifier was added inside a ROI however after a lot
# of improvements in teh dataset it was still not able to 
#  find teh signs, then we evolve to v2 using heuristics 
# to find candidates tehn applyin the CNN
# ============================================================
...
# The model performs full-frame classification. This means the bounding box
# drawn on screen represents the analyzed camera frame, not an object detector box.
...
def classify_sign_frame(frame_bgr, sign_model, sign_class_names):
    """
    Classify a upper-right ROI of the Webots camera frame using the CNN sign model.

    Signs appear in the upper-middle/right area of the camera view, so a tight
    ROI reduces background noise and improves classifier focus.
    The returned box reflects the actual ROI analyzed, not the full frame.
    """
    ...
    # Match the CNN training input size.
    roi_resized = cv2.resize(roi, SIGN_IMG_SIZE)

    # image_dataset_from_directory loads images as RGB.
    # OpenCV/Webots frame is BGR, so convert before prediction.
    roi_rgb = cv2.cvtColor(roi_resized, cv2.COLOR_BGR2RGB)

    # Model input shape: (batch, height, width, channels)
    input_tensor = np.expand_dims(roi_rgb.astype("float32"), axis=0)
...
def draw_sign_detection(debug_frame, label, confidence, box):
    """
    Draw the CNN sign result on the debug frame.

    Orange = sign class detected.
    Green = no_sign.
    """
    x, y, w, h = box
...
def main():
    ...
    while robot.step() != -1:
        ...
        # The classifier uses the full camera frame and returns one class.
        ...
        if (
            ...
            sign_label, sign_confidence, sign_box = classify_sign_frame(
                frame,
                sign_model,
                sign_class_names
            )
        ...
        # ==========================================
        # If confidence is high enough, draw full-frame bounding box and label.
        # If confidence is lower, show the predicted class as uncertain.
