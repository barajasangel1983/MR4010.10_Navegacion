...
# ROI grande → heurística red/white → crop candidato → CNN classifier → box + label
# ============================================================
...
def detect_red_white_sign_box(img_bgr):
    """
    Detect a candidate sign region inside an image/ROI using red and white
    color masks plus contour filtering.

    This is not the final classifier. It only proposes a candidate crop for
    the CNN. The CNN still decides whether it is stop, yield, speed,
    nopedestrian, or no_sign.

    Returns:
        box: (x, y, w, h) in local ROI coordinates, or None
        mask: binary mask used for debugging if needed
    """
    hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)

    # Red has two HSV ranges because hue wraps around 0/180.
    ...
    # White mask helps capture speed signs and sign interiors.
    ...
    # Clean the mask so contours are more stable.
    ...
    for contour in contours:
        ...
        # Remove tiny noise and reflections.
        if area < 80:
            continue

        # Remove huge objects like sky/building/road regions.
        if area > image_area * 0.40:
            continue

        aspect_ratio = w / max(h, 1)

        # Traffic signs are not extremely thin or extremely wide.
    ...
    x, y, w, h, _ = valid_boxes[0]

    return (x, y, w, h), mask


def crop_with_margin(img_bgr, box, margin_ratio=0.35):
    """
...
def classify_sign_frame(frame_bgr, sign_model, sign_class_names):
    """
    Hybrid sign detection and classification.

    Returns:
        label: predicted class name
        confidence: model probability
        sign_box: tight candidate box in full-frame coordinates
        search_roi_box: large green ROI box in full-frame coordinates
    """
    height, width, _ = frame_bgr.shape

    # Large search area where road signs normally appear in the camera frame.
    # This is only the search area, not the final CNN input.
    ...
    candidate_box, candidate_mask = detect_red_white_sign_box(roi)

    # If no candidate is found, classify the scene as no_sign.
    # sign_box is None because there is no tight candidate box to draw.
    ...
    # Convert expanded crop box from ROI coordinates to full-frame coordinates.
    ...
    # The model was trained from TensorFlow datasets, which use RGB.
    ...
    return label, confidence, sign_box, search_roi_box
...
def main():
    ...
    while robot.step() != -1:
        ...
        if ENABLE_SIGN_DETECTION and sign_box is not None:
            if sign_confidence >= SIGN_CONFIDENCE_THRESHOLD:
                draw_sign_detection(
                    debug_frame,
                    sign_label,
                    sign_confidence,
                ...
                cv2.putText(
                    debug_frame,
                    f"SIGN: uncertain ({sign_label} {sign_confidence:.2f})",
                    (30, 250),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (0, 255, 255),
