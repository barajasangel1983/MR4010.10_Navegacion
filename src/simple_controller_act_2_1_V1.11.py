...
#
#   Trigger 1 — evasion_phase 1 (EVADING):
#     BC (bc_evasion_model.pt) replaces PID to steer around detected vehicle.
#     Speed 35% AUTONOMOUS_SPEED. Hazard lights follow vehicle_nearby.
#     Toggle with key B.
#
#   Trigger 2 — evasion_phase 2 (RETURNING):
#     Sensor-based control — PID is blind in the right lane (no yellow line visible).
...
#     Speed 40% AUTONOMOUS_SPEED. Falls back to decay+countdown if BC not loaded.
#     10-second safety countdown always present — reverts to MANUAL if unresolved.
#
# LANE FOLLOWING
...
BC_ENABLED  = True   # set False to keep pure PID in all phases
BC_IMG_H    = 66
BC_IMG_W    = 200
BC_CROP_TOP = 0.38
BC_CROP_BOT = 0.88
...
_BC_DEVICE = None   # set once at load time


def _bc_load_model():
    global _BC_DEVICE
    if not BC_ENABLED or not _BC_TORCH_AVAILABLE:
        return None
    if not os.path.isfile(_BC_MODEL_PATH):
        print(f"[BC] Model file not found: {_BC_MODEL_PATH}")
        print("[BC] Train notebook 26 first to generate the model.")
        return None
    ...
    model.eval()
    print(f"[BC] Model loaded: {_BC_MODEL_PATH}  device={_BC_DEVICE}")
    return model


def _bc_predict(model, frame_bgr):
    """Return steering angle predicted by BC model for the current frame."""
...
def main():
    global DEBUG_PANEL, ADAS_MONITOR_ENABLED, BC_ENABLED
    speed = 0
    angle = 0.0
    brake = 0.0
    ...
    print(f"[BC] BC model: {'LOADED' if bc_model else 'NOT FOUND — pure PID'}")
    ...
    while robot.step() != -1:
        ...
        if key != -1:
            if key not in last_press or current_time - last_press[key] > DEBOUNCE_TIME:
                ...
                if key == keyboard.UP:
                    ...
                    print(f"[BC] Behavioral Cloning: {'ON' if BC_ENABLED else 'OFF (pure PID)'}")
        ...
        if autonomous_mode:
            ...
            # ==========================================
            # BC EVASION — phase 1 (EVADING) only
            # Phase 2 (RETURNING) handled by PID below — PID is better at lane-following
            # ==========================================
            bc_active = False
            ...
            # ── Phase 2 RETURNING: sensor-based stabilization + gradual left return ──
                ...
                # Car is in the right lane. PID is blind (can't see yellow line from here).
                # DS_MR keeps the car at safe distance from the right guard.
                # Steer left gradually until line_detected=True (state machine exits phase 2).
                bc_active = True  # mark handled so LIDAR/PID blocks are skipped
                hazard_lights_on = False
                brake = 0.0
                speed = int(RETURN_SPEED_RATIO * AUTONOMOUS_SPEED)

                # Base steering: gentle left to migrate back to left lane
                steer = RETURN_LANE_ANGLE

                # Right wall safety: if DS_MR too close, reduce left steer
                if ds_mr_val is not None:
                    if ds_mr_val < DS_RETURN_RIGHT_MIN:
                        # Too close to right guard — straighten out, prioritise safety
                        steer = max(steer, -0.04)
                    elif ds_mr_val < DS_RETURN_RIGHT_TARGET:
                        # Proportional correction: closer to target = less left steer
                ...
                # Right wall distance indicator in HUD
            ...
            # ==========================================
            # LIDAR SAFETY OVERRIDE (runs when BC is NOT active)
            # ==========================================
            ...
            if not bc_active and lidar_obstacle and lidar_obstacle_distance < LIDAR_MAX_DISTANCE:
                ...
                if line_detected:
                    ...
                    # ==========================================
                    # LINE LOST — BC fallback (intersections / crossings)
                    # ==========================================
                    ...
                    if BC_ENABLED and bc_model is not None:
                        # BC maintains heading through the crossing
                        try:
                            angle = _bc_predict(bc_model, frame)
                            angle = max(-MAX_ANGLE, min(MAX_ANGLE, angle))
                            previous_angle = angle
                            speed = int(0.40 * AUTONOMOUS_SPEED)
                            brake = 0.0
                            cv2.putText(
                                debug_frame,
                            ...
                            print(f"[BC] LINE_LOST inference error: {_e} — using decay")
                            angle = previous_angle * 0.5
                            speed = 10
                    else:
                        # No BC available — original decay fallback
                        angle = previous_angle * 0.5
                        speed = 10
                        if abs(angle) < 0.03:
                            angle = 0.0
                    ...
                    # Safety countdown — always visible regardless of BC state
                    countdown_color = (0, 100, 255) if countdown > 3 else (0, 0, 255)
                    cv2.putText(
                        debug_frame,
                        f"Line lost — auto MANUAL in: {countdown}s",
                        (30, 75),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.65,
                        countdown_color,
                    ...
                    # Safety net — revert to MANUAL if line never comes back
                    if time_remaining <= 0:
                        autonomous_mode = False
                        line_lost_time = None
                        pid.reset()
                        previous_angle = 0.0
                        angle = 0.0
                        print("Line lost 10 s — automatically reverted to MANUAL mode")
