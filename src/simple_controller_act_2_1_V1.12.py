# ============================================================
# V1.12 — Webots Self-Driving Controller
# ============================================================
#
# WHAT IS NEW IN V1.12 vs V1.11
# ──────────────────────────────
#   1. BC model upgraded to bc_behavior_model.pt — trained on 5 behavior classes
#      (0=NORMAL, 1=EVADING, 2=RETURNING, 3=SHARP_LEFT, 4=SHARP_RIGHT) using
#      dataset_mode_v4 with the balanced inverse-frequency weight sampler.
#   2. Two independent runtime toggles:
#        B → enable/disable BC inference
#        L → enable/disable PID / line-follower
#      This creates four operating modes (see BC / PID ARBITRATION below).
#   3. Per-class speed targets in BC mode (BC_SPEED_BY_CLASS dict).
#   4. BC output EMA smoothing (BC_SMOOTH_ALPHA) + angle clamping (BC_ANGLE_CLAMP).
#   5. Stuck-vehicle detection: warns in console if actual speed < 5 km/h
#      for > 5 s while BC is commanding movement.
#   6. ADAS Monitor updated with BC / PID status rows.
#   7. Dataset capture (dataset_mode_v4) is IDENTICAL to V1.11 — both files
#      write to the same behavioral_dataset_06232026 directory. Use V1.11 for
#      pure expert-data collection; use V1.12 when you want to collect data
#      AND test BC inference in the same Webots run.
#
...
import dataset_mode_v4 as dataset_mode
...
def draw_adas_monitor(speed, angle, brake, gps_vals, gyro_vals,
    ...
    # Panel height increased by 46 px vs V1.11 to accommodate BC / PID status rows
    W, H = 400, 879
    ...
    # Yellow line — shifted +46 px from V1.11
    ...
    data_row(187, "Yellow line", line_str, line_color)

    # wZ alignment row — only visible during RETURNING; shifted +46 px
    if evasion_phase == 2 and saved_gyro_wz is not None and current_gyro_wz is not None:
        ...
        data_row(210, "wZ align", wz_str, wz_color)

    cv2.line(panel, (8, 224), (W - 8, 224), (50, 50, 50), 1)

    # ── VEHICLE STATE — shifted +46 px ────────────────────────────────────
    section_header(237, "VEHICLE STATE")
    spd_color = (0, 255, 0) if speed < 60 else (0, 165, 255)
    brk_color = (0, 0, 255) if brake > 0.1 else (210, 210, 210)
    data_row(259, "Speed",  f"{speed:.1f}  km/h", spd_color)
    data_row(282, "Angle",  f"{angle:.4f}  rad")
    data_row(305, "Brake",  f"{brake:.2f}", brk_color)

    # ── GPS — shifted +46 px ──────────────────────────────────────────────
    section_header(333, "GPS")
    if gps_vals is not None:
        data_row(355, "X", f"{gps_vals[0]:.3f}  m")
        data_row(378, "Y", f"{gps_vals[1]:.3f}  m")
        data_row(401, "Z", f"{gps_vals[2]:.3f}  m")
    else:
        data_row(365, "Status", "NOT AVAILABLE", (0, 80, 255))

    # ── GYRO — shifted +46 px ─────────────────────────────────────────────
    section_header(429, "GYRO")
    if gyro_vals is not None:
        data_row(451, "wX", f"{gyro_vals[0]:.4f}  rad/s")
        data_row(474, "wY", f"{gyro_vals[1]:.4f}  rad/s")
        data_row(497, "wZ", f"{gyro_vals[2]:.4f}  rad/s")
    else:
        data_row(461, "Status", "NOT AVAILABLE", (0, 80, 255))

    # ── DISTANCE SENSORS — shifted +46 px ─────────────────────────────────
    section_header(525, "DISTANCE SENSORS")
    ...
    data_row(547, "DS_ML  (mid-left)",    ml_str, ds_color(ds_ml_val))
    data_row(570, "DS_MR  (mid-right)",   mr_str, ds_color(ds_mr_val))
    data_row(593, "DS_FL  (front-left)",  fl_str, ds_color(ds_fl_val))
    data_row(616, "DS_FR  (front-right)", fr_str, ds_color(ds_fr_val))
    data_row(639, "DS_RL  (rear-left)",   rl_str, ds_color(ds_rl_val))
    data_row(662, "DS_RR  (rear-right)",  rr_str, ds_color(ds_rr_val))

    # ── LIDAR — shifted +46 px ────────────────────────────────────────────
    section_header(690, "LIDAR  (Sick LMS 291)")

    if lidar_obstacle and lidar_distance is not None:
        dist_color = (0, 0, 255) if lidar_distance < LIDAR_STOP_DISTANCE else (0, 165, 255)
        data_row(712, "Status",   "OBSTACLE DETECTED", dist_color)
        data_row(735, "Distance", f"{lidar_distance:.2f}  m", dist_color)
        ang_str = f"{lidar_angle:.1f}  deg" if lidar_angle is not None else "--"
        data_row(758, "Angle",    ang_str, (210, 210, 210))
        type_color = (0, 100, 255) if obstacle_type == "VEHICLE" else (210, 210, 210)
        data_row(781, "Type",     obstacle_type if obstacle_type else "--", type_color)
        vh_str = "YES" if vehicle_nearby else "NO"
        vh_color = (0, 0, 255) if vehicle_nearby else (0, 200, 0)
        data_row(804, "Vehicle",  vh_str, vh_color)
    else:
        data_row(712, "Status",   "CLEAR", (0, 255, 0))
        data_row(735, "Distance", "--")
        data_row(758, "Angle",    "--")
        data_row(781, "Type",     "--")
        data_row(804, "Vehicle",  "NO", (0, 200, 0))
...
def main():
    ...
    print("=" * 55)
    print("  Webots Self-Driving Controller — V1.12")
    print("=" * 55)
