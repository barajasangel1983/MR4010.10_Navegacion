# ============================================================
# V2.0 — Webots Self-Driving Controller
# ============================================================
#
...
# Path to the TorchScript CIL model — NB30 architecture, trained externally.
_CIL_MODEL_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "models", "cil_model3.pt"
)
...
def _cil_preprocess(frame_bgr):
    ...
    Must match CILDataset._load_and_crop in Notebook 30 exactly:
...
def _cil_predict_smooth(model, frame_bgr, nav_command, prev_smoothed):
    ...
    nav_cmd_t = _torch.tensor([nav_command], dtype=_torch.long, device=tensor.device)
    with _torch.no_grad():
        raw = float(model(tensor, nav_cmd_t).item())
...
def main():
    ...
    print("=" * 60)
    print("  Webots Self-Driving Controller — V2.0")
    print("=" * 60)
